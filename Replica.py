...
class Replica(object):
	...
	def __init__(self, sim, seed=1,
		...
		## service time (mean and standard deviation) with optional content
		...
		## The amount of time this server is active. Useful to compute
		# utilization. Since we are in a simulator, this value is hard to use
		# correctly. Use getActiveTime() instead.
		self._activeTime = 0
		## The time when the server became last active (None, not currently
		# active)
	...
	def _runReportLoop(self):
		# Compute utilization
		utilization = ((self.getActiveTime() - self._lastActiveTime) /
			self.reportPeriod)
		...
		self._sim.report(self,
			...
			(avg(self._latestLatencies), maxOrNan(self._latestLatencies),
			utilization),
		)
	...
	## Serve a request
	# @param requestId An opaque value used to identify this request
	# @param replyTo Call this object's reply() method
	# @param headers Dictionary of values that may influence the replica's
	#   behaviour. Currently, only "withOptional" is taken into account.
	# @note When request completes an event is scheduled to:
	#   replyTo.reply(requestId, headers).
