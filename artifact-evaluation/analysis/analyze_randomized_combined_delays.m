...
if ~isfile('randomized_combined_data.mat')
    ...
    for k = 1:m
        ...
        descr =  read_combined_description_file([path '/' tests(k).name '/' 'description.csv']);
        dataCell{k, 2} = descr;
        result = read_final_result_file([path '/' tests(k).name '/' 'sim-final-results.csv']);
        dataCell{k, 3} = result;
    end
    
    save('randomized_combined_data.mat', 'dataCell');
else
    tmp = load('randomized_combined_data.mat');
...
for i = 1:m
    if dataCell{i, 2}.delayFrac < 0.007
        continue
    end
    ...
    if mu_bound > dataCell{i, 2}.nbrServer*dataCell{i, 2}.lambdaFrac
        bound_c = 1/(mu_bound - dataCell{i, 2}.nbrServer*dataCell{i, 2}.lambdaFrac);
    else
        bound_c = NaN;
    end
    bound_a = dataCell{i, 2}.arrivalDelay;
    bound = bound_a + bound_c;
    ...
    if dataCell{i, 2}.nbrServer < 11
        ...
        idx_frac = find(abs(delayFracs-dataCell{i, 2}.delayFrac)<1e-5);
