...
if ~isfile('randomized_arrival_data.mat')
    path = '../simulation-results/randomized-arrival-delays';
    tests = dir(path);
    tests(1:2) = [];
    ...
    for k = 1:m
        ...
        descr =  read_arrival_description_file([path '/' tests(k).name '/' 'description.csv']);
        dataCell{k, 2} = descr;
        result = read_final_result_file([path '/' tests(k).name '/' 'sim-final-results.csv']);
        dataCell{k, 3} = result;
    end
    
    save('randomized_arrival_data.mat', 'dataCell');
    
else
    tmp = load('randomized_arrival_data.mat');
...
for i = 1:2200
    ...
    if (dataCell{i, 2}.nbrServer < 11) && (dataCell{i, 2}.arrivalDelay > 0.005)
        idx_util = find(abs(utils-dataCell{i, 2}.util)<1e-5);
        idx_frac = find(abs(delayFracs-dataCell{i, 2}.arrivalDelay/dataCell{i, 2}.meanServiceTime)<1e-5);
...
for l=1:length(resultCell)
    avg_bound_errors(l) = mean(resultCell{l, 1});
...
fileID = fopen('../plots/data/randomized-delays/randomized_arrival_delays_confint_bound.txt','w');
fprintf(fileID,'%6.4f %6.4f %6.4f %6.4f\n',(conf_bound_errors)');
fclose(fileID);

fileID = fopen('../plots/data/randomized-delays/randomized_arrival_delays_confint_resp.txt','w');
fprintf(fileID,'%6.4f %6.4f %6.4f %6.4f\n',(conf_resp_errors)');
fclose(fileID);
