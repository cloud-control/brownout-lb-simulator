...
class MMReplicaController:
	def __init__(self, sim, name, period, K, \
		Ti, usePI, seed = 1):
	...
	def runControlLoop(self):
		...
		if self.latestLatencies:
			...
			if len(self.latestLongLatencies) > 0:
				...
				feedbackMin = -1.0*self.queueLength
				self.v = max(prelFeedback, feedbackMin)
