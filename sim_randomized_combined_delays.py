...
delayFracs = [0.001, 0.005, 0.01, 0.02, 0.05, 0.1, 0.2, 0.5]
...
def getRandomStableFracs(delayFrac, util):
    stableCancellationFrac = 0.95 - util
...
for scenario in NBR_SCENARIOS:
    ...
    simulations.append("./simulator.py  --lb clone-random --scenario scenarios/clone-PS-randomized.py --cloning 1 --nbrClones {} \
        ...
        --setSeed {} --maxRunTime {} --outdir result/randomized_combined_delays/scenario{} \
        ".format(cloneFactor, arrivalDelay, cancellationDelay, dist, frac, nbrServer, count*100 + 123456, MAXRUNTIME, scenario))

    with open("result/randomized_combined_delays/scenario{}/description.csv".format(scenario), 'a') as f:
